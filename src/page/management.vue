    ...
    <div class = 'management'>
            ...
            <div class = 'right'>
                ...
                <el-dropdown trigger="click" v-if = 'type == 2' class = 'addBtn' style = 'display:inline-block' @command="handleCommand">
                    ...
                    <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item command = '5'>押运员</el-dropdown-item>
                        <el-dropdown-item command = '6'>驾驶员</el-dropdown-item>
                    </el-dropdown-menu>
                </el-dropdown>
                <div class = 'serBox'>
        ...
        <el-dialog :title="title" :visible.sync="addNew" :before-close = 'close' :close-on-click-modal = "canClose" :close-on-press-escape = 'canClose'>
            <el-form :model="newForm" ref = "newForm" :rules="rules"  :disabled = 'isDetail' v-if = 'addNew'>
                ...
                <el-row :gutter="40">
                    <el-col :span="12">
                        <el-form-item label="人员类型" label-width="80" prop = 'type' :rules="[{ required: true, message: '人员类型不能为空'}]" >
                            <el-select v-model="newForm.type" :disabled = '!isEdit && type == 2' @change = 'typeChange'>
                                <el-option :label="v.text" :value="v.value" :key = 'i' v-for = "(v,i) in personType" v-if = "v.value != '-1' && v.value != 9"></el-option>
                            </el-select>
                        </el-form-item>
                        <!-- 运输 -->
                        <el-form-item label="驾驶证号" label-width="80" prop = 'drivingLicence' v-if = 'newForm.type == 6'>
                            <el-input v-model="newForm.drivingLicence" @keyup.native="newForm.identityId = newForm.identityId.replace(/[^0-9A-Za-z]/g,'')"></el-input>
                        </el-form-item>
                        <!-- 运输 -->
                        <el-form-item label="从业资格证号" label-width="80" prop = 'qualificationLicence' v-if = 'type == 2' :rules = "[{ required: true, message: '从业资格证号不能为空'}]">
                            <el-input v-model="newForm.qualificationLicence"  @keyup.native="newForm.qualificationLicence = newForm.qualificationLicence.replace(/\D/g,'')"></el-input>
                        </el-form-item>
                        <!-- 其他公司 -->
                        <el-form-item label="作业证号" label-width="80" prop = 'licenceNumber' v-if = 'type != 2'>
                            <el-input v-model="newForm.licenceNumber"></el-input>
                        </el-form-item>
                        <el-form-item label="劳务合同到期时间" label-width="80" prop = 'laborContractIndate' :rules="[{ required: true, message: '劳务合同到期时间不能为空'}]">
                            <el-date-picker type="date" v-model="newForm.laborContractIndate" style="width: 100%;"></el-date-picker>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label="作业许可级别" label-width="80" prop = 'staffLevel' :rules="[{ required: true, message: '作业证级别不能为空'}]" v-if = 'newForm.type == 4 || newForm.type == 8' >
                            <el-select v-model="newForm.staffLevel">
                                <el-option :label="v" :value="i" :key = 'i' v-for = "(v,i) in level"></el-option>
                            </el-select>
                        </el-form-item>
                        <el-form-item label="驾驶证到期时间" label-width="80" prop = 'drivingLicenceIndate' :rules="[{ required: true, message: '驾驶证到期时间不能为空'}]" v-if = 'newForm.type == 6' >
                            <el-date-picker type="date" v-model="newForm.drivingLicenceIndate" style="width: 100%;"></el-date-picker>
                        </el-form-item>
                        <el-form-item label="从业资格到期时间" label-width="80" prop = 'qualificationLicenceIndate' :rules="[{ required: true, message: '从业资格到期时间不能为空'}]" v-if = 'type == 2'>
                            <el-date-picker type="date" v-model="newForm.qualificationLicenceIndate" style="width: 100%;"></el-date-picker>
                        </el-form-item>
                        <el-form-item label="作业证书到期时间" label-width="80" prop = 'licenceNumberIndate' :rules="[{ required: true, message: '作业证书到期时间不能为空'}]" v-if = 'type != 2'>
                            <el-date-picker type="date" v-model="newForm.licenceNumberIndate" style="width: 100%;"></el-date-picker>
                        </el-form-item>
                        <el-form-item label="责任书到期时间" label-width="80" prop = 'responsibilityAgreementIndate' :rules="[{ required: true, message: '责任书到期时间不能为空'}]">
                            <el-date-picker type="date" v-model="newForm.responsibilityAgreementIndate" style="width: 100%;"></el-date-picker>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="40">
                    <el-col :span = '8'>
                        <el-form-item label="人员照片" prop="imgs[0].photo" :rules="[{ required: true, message: '请上传照片'}]">
                            <File :data="upload[0]" @bindValue="newForm.imgs[0].photo = upload[0].url" :look = 'isDetail'></File>
                        </el-form-item>
                    </el-col>
                    <el-col :span = '8'>
                        <el-form-item label="身份证照片（人像面）" prop="imgs[1].photo" :rules="[{ required: true, message: '请上传照片'}]">
                            <File :data="upload[1]" @bindValue="newForm.imgs[1].photo = upload[1].url" :look = 'isDetail'></File>
                        </el-form-item>
                    </el-col>
                    <el-col :span = '8'>
                        <el-form-item label="身份证照片（国徽面）" prop="imgs[2].photo" :rules="[{ required: true, message: '请上传照片'}]">
                            <File :data="upload[2]" @bindValue="newForm.imgs[2].photo = upload[2].url" :look = 'isDetail'></File>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="40">
                    <el-col :span = '8'>
                        <el-form-item label="作业证书照片" prop="imgs[3].photo" :rules="[{ required: true, message: '请上传照片'}]" v-if = 'type != 2'>
                            <File :data="upload[3]" @bindValue="newForm.imgs[3].photo = upload[3].url" :look = 'isDetail'></File>
                        </el-form-item>
                        <el-form-item label="从业资格照片" prop="imgs[3].photo" :rules="[{ required: true, message: '请上传照片'}]" v-if = 'type == 2'>
                            <File :data="upload[3]" @bindValue="newForm.imgs[3].photo = upload[3].url" :look = 'isDetail'></File>
                        </el-form-item>
                    </el-col>
                    <el-col :span = '8'>
                        <el-form-item label="劳务合同照片（多图）" prop="imgs[4].photoList" :rules="[{ required: true, message: '请上传照片'}]">
                            <File :data="upload[4]" @bindValue="newForm.imgs[4].photoList = upload[4].url" more = 'true' :look = 'isDetail'></File>
                        </el-form-item>
                    </el-col>
                    <el-col :span = '8'>
                        <el-form-item label="责任书照片（多图）" prop="imgs[5].photoList" :rules="[{ required: true, message: '请上传照片'}]">
                            <File :data="upload[5]" @bindValue="newForm.imgs[5].photoList = upload[5].url" more = 'true' :look = 'isDetail'></File>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="40">
                    <el-col :span = '8'>
                        <el-form-item label="驾驶证照片（正面）" prop="imgs[6].photo" :rules="[{ required: true, message: '请上传照片'}]" v-if = 'newForm.type == 6'>
                            <File :data="upload[6]" @bindValue="newForm.imgs[6].photo = upload[6].url" :look = 'isDetail'></File>
                        </el-form-item>
                       
                    </el-col>
                    <el-col :span = '8'>
                        <el-form-item label="驾驶证照片（反面）" prop="imgs[7].photo" :rules="[{ required: true, message: '请上传照片'}]" v-if = 'newForm.type == 6'>
                            <File :data="upload[7]" @bindValue="newForm.imgs[7].photo = upload[7].url" :look = 'isDetail'></File>
                        </el-form-item>
                    </el-col>
